...
import { removeProduct, setBackStep, setNextStep, setShippingCharge, setStep, updateProduct } from 'store/reducers/cart';
...
//  const { addresses } = useSelector((state) => state.product);
  ...
  const handleChange = (newValue) => {
    ...
    dispatch(setStep(newValue === 0 ? newValue : newValue + 1));
  };

  useEffect(() => {
    setValue(cart.checkout.step > 1 ? 1 : cart.checkout.step);
